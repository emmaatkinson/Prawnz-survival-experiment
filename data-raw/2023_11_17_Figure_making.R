...
#Read in Model
models_list <-readRDS("models.rda")
...
par(mfrow=c(1,1),mar=c(4,4,1,2), oma=c(0,0,4,0))
barplot(lost_prawnz, ylim=c(-5,70),names=as.character(unique(trial$trial_number)), xlab="Trial number", ylab="Lost prawns",col=viridis(4,0.5)[1])
dev.off()
...
sum(model_df_2$stage==0,na.rm=TRUE)
...
barplot(100*barplot_df2, beside=TRUE, col=viridis(2)[1:2],
        ...
        xlab="Time out of Water (minutes)",ylab="Percent of prawns", ylim=c(0,100))
legend("right",c("Alive","Dead"), fill=c(viridis(2)[1:2]), cex=0.8)
dev.off()
...
Bootstrap<-function(DATA,){
  ...
  for (i in 1:1000){
    boot[i]<- mean(sample(DATA, size=length(DATA), replace=TRUE))
  }
  return(quantile(boot, probs=c(0.025),mean(DATA),quantile(boot, probs=c(0.975))))
}
dev.off()
...
cols<-viridis(2,0.7)
plot(hgA, col = cols[1], xlab="Length (mm)", ylab="Number of prawns", xaxt='n') # Plot 1st histogram using a transparent color
...
library(lme4)
model_6.1_1<-glmer(alive~temp*length+treatment*temp+(1|trial_trap),data=model_df_2,family=binomial,nAGQ=10)
