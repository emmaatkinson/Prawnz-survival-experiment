...
for (i in 1:5){
  ...
  if (i == 4){
    ...
    current_data<-model_df_2[c(which(model_df_2$treatment==90),which(model_df_2$treatment==100)),]$alive
    means[i]<-mean(current_data)
    boot<-vector(length=1000)
  ...
  if (i == 5){
    ...
    current_data<-model_df_2[which(model_df_2$treatment==120),]$alive
    means[i]<-mean(current_data)
    boot<-vector(length=1000)
...
hgA<-hist(model_df_2[which(model_df_2$alive==1),]$length,breaks = pretty(17.5:53.5, n = 69), plot=FALSE)
hgB<-hist(model_df_2[which(model_df_2$alive==0),]$length,breaks = pretty(17.5:53.5, n = 69), plot=FALSE)
...
seen.alive[1]<-mean(model_df_2[which(model_df_2$treatment==0),]$alive)*100
seen.alive[2]<-mean(model_df_2[which(model_df_2$treatment==30),]$alive)*100
seen.alive[3]<-mean(model_df_2[which(model_df_2$treatment==60),]$alive)*100
seen.alive[4]<-mean(model_df_2[which(model_df_2$treatment==90),]$alive)*100
seen.alive[5]<-mean(model_df_2[which(model_df_2$treatment==120),]$alive)*100
